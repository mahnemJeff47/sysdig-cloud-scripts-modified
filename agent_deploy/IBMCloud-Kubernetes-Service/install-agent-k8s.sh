...
function install_k8s_agent {
    ...
    ICR_SECRET_EXIST=$(kubectl -n default get secret default-us-icr-io >/dev/null 2>&1 || echo 1)
    if [ "$ICR_SECRET_EXIST" = 1 ]; then
        ...
        echo "ERROR: default-us-icr-io secret doesn't exist in the default namespace"
    ...
    kubectl get secret default-us-icr-io -n default -o yaml | sed "s/default/$NAMESPACE/" | kubectl apply -n $NAMESPACE -f -
